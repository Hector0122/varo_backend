...
### FinancialObjective V2 — hecho (backend y frontend)
...
**Fase 2 (frontend + API unificada), también hecha**: se expuso `FinancialObjectivesController` en `/objectives` (`GET /objectives?type=`, `GET/PATCH/DELETE /objectives/:id`, `POST /objectives`, `POST /objectives/:id/entries`, `GET /objectives/:id/entries`, `GET /objectives/:id/forecast`, `GET /objectives/spending/monthly`), y `GoalDetailScreen`/`DebtDetailScreen`/`GoalCard`/`DebtCard`/`GoalsScreen`/`DashboardScreen` en `varo_frontend` migraron a este endpoint compartiendo el tipo `FinancialObjective`. Las pantallas se mantuvieron separadas a propósito (Meta y Deuda tienen UX genuinamente distinta — agregar/retirar vs. pagar/incrementar con installments y ciclo de facturación); lo que se unificó fue el modelo de datos y la capa de API, no la UI. `GoalsController`/`DebtController` y las rutas `/goals`/`/debts` se eliminaron por completo una vez migrado el frontend — cutover directo sin período de soporte en paralelo, dado que esta app tiene un solo consumidor de su API.
